...
local install_cmds = {
  ...
  brew = 'brew install codex', -- official docs recommend core formula
}
...
local fallback_instructions = {
  ...
  -- brew installs into /opt/homebrew/bin or /usr/local/bin which are usually on PATH.
  -- No fallback needed.
}
...
function M.detect_available_package_managers()
  local pm_list = { 'npm', 'pnpm', 'yarn', 'bun', 'deno', 'brew' }
  local available = {}
      ...
      if code == 0 then
        ...
        if not M.__test_ignore_path_check and vim.fn.executable 'codex' == 0 then
          ...
          if fallback then
            ...
            vim.notify('[codex.nvim] CLI not found in PATH after install. Try restarting your shell.',
              vim.log.levels.WARN)
          end
        end
        if on_success then
...
function M.prompt_autoinstall(on_done)
  ...
  vim.schedule(function()
    vim.ui.select(pms, {
      prompt = 'Select a package manager to install @openai/codex:',
      kind = 'codex-install',
    }, function(choice)
      if not choice then
        on_done(false)
